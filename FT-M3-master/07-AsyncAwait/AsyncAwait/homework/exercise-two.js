...
async function problemA () {
  ...
  await Promise.all([readAsync('poem-two/stanza-01.txt'), readAsync('poem-two/stanza-02.txt')])//promise all nos sirve para mandar en simultaneo las asyncronicas
  console.log('done')
}
...
async function readAsync(file){
  ...
  const arrayPromises = filenames.map( f=> readAsync(f))
