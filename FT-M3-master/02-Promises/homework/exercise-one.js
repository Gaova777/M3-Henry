...
promisifiedReadFile('poem-one/stanza-01.txt')
  // .then(st => { //resultado de la lecutra
  //   blue(st)
  // })
  .then(blue) //una version extremadamente corta, donde blue se activa con lo que le paso a .then 
}
...
function problemC () {
  ...
  .then(st2 => {
    blue(st2)
    return promisifiedReadFile('poem-one/stanza-03.txt')// :OOO increible
  })// el valor retornado será recibido por el siguiente .then
  .then(st3 => {
    blue(st3)
    console.log('done')
  })
  ...
  .then(st2 => {
    blue(st2)
    promisifiedReadFile('poem-one/stanza-03.txt')// :OOO increible
  .then(st3 => {
    blue(st3)
    console.log('done')
  }) --> el problema de hacerlo anidado es cuando se tiene muchos anidados me va a resultar retornando valores anteriores y eso no es lo que queremos, por eso es mejor hacerlo con el return
  })// el valor retornado será recibido por el siguiente .then
   */
}
...
function problemE () {
  ...
  // promise version
  // ???
  promisifiedReadFile('poem-one/stanza-faef03.txt')
  .then(st3=> {
    blue(st3)
    return promisifiedReadFile('poem-one/standaea.txt')// :OOO increible
  })// el valor retornado será recibido por el siguiente .then
  .then(st4 => {
    blue(st4)
    console.log('done')
...
function problemF () {
  ...
  .then(st3=> {
    blue(st3)
    return promisifiedReadFile('poem-one/stanza-04.txt')// :OOO increible
  })// el valor retornado será recibido por el siguiente .then
  .then(st4 => {
    blue(st4)
   // console.log('done')
  ...
  //bloque finally, haya tomado el camino que haya tomado, cuando termine, haceme lo sgte (una forma de ahorrar lineas de codigo, mireque comentamos los consoles.log -->
  .finally(()=>{
    console.log('done')
  })
}
